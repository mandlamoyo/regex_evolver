...
class ReType:
    ...
    def is_type(self, re_type):
        return self.name == re_type.name or (self.parent and self.parent.is_type(re_type))

    def is_type_name(self, type_name):
        return self.name == type_name or (self.parent and self.parent.is_type_name(type_name))
...
class ReNode:
    ...
    @staticmethod
    def make_random_node(type_name='re', mpr=CHANCE_MODIFIER, omit=None):
        ...
        if omit:
            ...
            filtered_rw_list = list(filter(lambda rw: not rw.re_type.is_type(omit_type), filtered_rw_list))
        rw = choice(filtered_rw_list)

        # create node of correct type
        children = [ReNode.make_random_node(choice(rw.child_types)) for i in range(rw.get_childcount())]
        node = ReNode(rw, children)
    ...
    def mutate(self, prob_change=0.1):
        if random() < prob_change:
            return ReNode.make_random_node(type_name=self.re_type.name)
        else:
            new_children = []
    ...
    def trailing_non_modifiable(self, child):
        if child.modifier: return True
        if child.re_type.is_type_name('mod'): return True
        if not child.re_type.is_modifiable: return True
        if child.children and self.trailing_non_modifiable(child.children[-1]): return True
        return False
...
class GeneticAlgorithm:
    ...
    def score_func(self, node_set, sample_size=None, verbose=False):
        correct = 0
        
        for row in self.sample_dataset(sample_size):
            res = GeneticAlgorithm.check_match(node_set.display(), row[0])
            if res == row[1]: correct += 1
            if verbose: print('> ', node_set.display(), row[0], row[1], res)
        if verbose:
            print('>> ', correct, len(self.dataset), correct/len(self.dataset), 1-correct/len(self.dataset))
            print()
        return 1 - correct/len(self.dataset)

    def rank_population(self, sample_size=None, verbose=False):
        population_sample = self.sample_population(sample_size)
        scores = [(self.score_func(node_set, verbose), node_set) for node_set in population_sample]
    ...
    def evolve(self, pop_size, max_gen=10, mutation_rate=0.2, breeding_rate=0.4, pexp=0.7, pnew_upper=0.9, pnew_lower=0.6):
        ...
        for i in range(max_gen):
            ...
            while len(new_pop) < pop_size:
                if random() < pnew:
                    ...
                    ix = GeneticAlgorithm.select_index(len(self.population)-1)
                    new_pop.append(scores[ix][1].mutate(mutation_rate))
...
rw_list.extend([
    rangew, setw, nsetw, countw, count2w, 
    orw, wildw, zeroplusw, zeroonew, oneplusw, ngreedw, 
    wspw, nwspw, empstw, empnstw, decw, ndecw, wordw, nwordw
])

def gen_postcode_test_match():
    pc = choice(CHAR_SETS['alpha'])
    if random() < 0.5: pc += choice(CHAR_SETS['alpha'])
    pc += choice(CHAR_SETS['num'])
    if random() < 0.5: pc += choice(CHAR_SETS['num'])
    pc += ' ' + choice(CHAR_SETS['num']) + choice(CHAR_SETS['alpha']) + choice(CHAR_SETS['alpha'])
    return pc
...
def gen_postcode_test_no_match(num_words, chars, prob_alphanum=0.5, prob_rightsize=0.5):
    ...
    if num_words == RAND: num_words = GeneticAlgorithm.select_index(MAX_WORDS)+1

    res = []
    if rightsize: num_words = 2
    for n in range(num_words):
        if rightsize: num_chars = [randint(3,4), 3][n]
        else: num_chars = GeneticAlgorithm.select_index(10)+1
...
def gen_postcode_test_data(n=1, prob_match=0.5, char_set=RAND):
    if char_set == RAND: char_set = list(CHAR_SETS.keys())
    else: char_set = [char_set]
    return [gen_postcode_test_datum(prob_match, chars=choice(char_set)) for i in range(n)]
